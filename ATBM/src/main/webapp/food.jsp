						...
						<form method="POST" action="CartController">
							<div class="col-md-4 col-sm-2 product">
								<div class="card  is-table-row" style="width: 14.5rem;">
									<a href="product?proId=${product.id }"> <input
										type="hidden" name="proId" value="${product.id}"> <input
					...
					<ul class="pagination">
						<c:if test="${tag >1}">
							<li class="page-item"><a class="page-link"
								href="FoodController?index=${tag-1}">Previous</a></li>
						</c:if>
						...
						<c:if test="${tag < endPFood}">
							<li class="page-item"><a class="page-link"
								href="FoodController?index=${tag+1}">Next</a></li>
						</c:if>
					</ul>
				</div>
			</div>
